...
const CategorySpecsForm = ({ data, update, onBack, onNext }) => {
  const specsConfig = {
    panel: [
      { name: 'wattage', label: 'Wattage (Wp)' }, 
      { name: 'efficiency', label: 'Efficiency (%)' }
    ],
    inverter: [
      { name: 'power', label: 'Power (kW)' }, 
      { name: 'mppt', label: 'MPPT Count' }
    ],
    motor: [
      { name: 'hp', label: 'Horsepower (HP)' }
    ]
  };
    ...
    <div className="space-y-6">
      ...
      {(specsConfig[data.category] || []).map(field => (
        <InputField
          key={field.name} 
          label={field.label} 
          // If editing, this shows existing data. If creating, it shows empty/default.
          value={data.specs?.[field.name] || ''} 
          onChange={(v) => update({ 
            ...data, 
            specs: { ...data.specs, [field.name]: v }
          })} 
        />
      ))}
