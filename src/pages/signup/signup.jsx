...
const Signup = () => {
  ...
  // 2. Destructure 'signup' from the context
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // 3. Call the Supabase signup function from your AuthProvider
      await signup(data.email, data.password, data.fullName);
      
      // Success: Let the user know and redirect
      alert("Account created successfully! Check your email to confirm.");
      navigate('/login'); 
    } catch (err) {
      // Catch and display Supabase errors (e.g., "User already exists")
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="max-w-xl w-full bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 p-8 lg:p-16 border border-slate-100">
        ...
        <div className="text-center mb-10">
          <div onClick={() => navigate("/")} className="inline-flex bg-emerald-600 p-3 rounded-2xl text-white mb-6 cursor-pointer hover:scale-105 transition-transform">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSignup}>
          <div className="space-y-2 col-span-2 md:col-span-1">
            ...
            <div className="relative">
              ...
              <input 
                type="text" required placeholder="Abrar Ahmed" 
          ...
          <div className="space-y-2 col-span-2 md:col-span-1">
            ...
            <div className="relative">
              ...
              <input 
                type="email" required placeholder="abrar@example.com" 
                className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-emerald-500 focus:bg-white focus:outline-none transition" 
          ...
          >
            {loading ? <Loader2 className="animate-spin" /> : <>Create My Account <ArrowRight size={20} /></>}
          </button>
        </form>

        <p className="mt-8 text-center text-sm text-slate-400 font-medium">
          Already a member? <Link to="/login" className="text-slate-900 font-bold hover:text-emerald-600 underline underline-offset-4 transition-colors">Log in here</Link>
        </p>
      </div>
    </div>
  );
